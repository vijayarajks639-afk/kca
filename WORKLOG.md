...
## WP-24 — Reuse measurement (E6 — Portability proof)
- **Branch:** `wp-24-reuse-measurement` (card's canonical name).
- **Base:** off `main` (`0ce76f0`, post-WP-23) in worktree `Projects/kca-wp24`.
  Dep WP-22 (op-risk = domain #2) merged.
- **Deliverables:** the published table `docs/reuse-measurement.md` (the card's
  package is docs/) + a small **measurement tool** `kca/evals/reuse/` that COMPUTES
  the numbers from the repo. **Flagged:** the tool is code under kca/evals (beyond
  the docs/ package) — added so the numbers are computed, not hand-typed or
  LLM-estimated (rule 2); the doc is regenerable and a test asserts it stays in sync.
  No new deps, no contracts changes.

**What it measures** (two complementary views, both from the repo):
- *Code footprint (size):* non-blank production LOC per area. Reusable substrate
  (kca/platform, contracts, services, data, apps, evals harness/judge/traps) =
  **7,609 LOC**; op-risk DIP (all under kca/dips, WP-22) = **699 LOC** (+190 tests).
  **Reused fraction 91.6%**; op-risk's marginal footprint is **9%** of the substrate.
- *Architecture (components):* from WP-22's `portability_report()` — **8** journey
  component roles are the identical platform module for both domains (reused spine),
  **4** are op-risk's own DIP assets; **0** platform components and **0** files
  outside kca/dips were added for op-risk.
- *Cost frame:* domain #1 built the platform (~7,609 LOC) + credit DIP; domain #2
  reused the platform unchanged (0 lines, 0 migrations, 0 contract schemas) and paid
  only for its DIP. Marginal cost of domain #2 ≈ its DIP = 9% of the substrate.

**Acceptance criteria → evidence:**
- *Reuse table published* → `docs/reuse-measurement.md` (both tables + cost frame),
  regenerated by `python -m kca.evals.reuse`; `test_measure.py` asserts the committed
  doc equals the tool output (no drift, no hand-editing).
- *Marginal-cost claim supported or honestly refuted* → **SUPPORTED**, computed:
  `claim_supported` requires 0 platform components added, nothing outside kca/dips,
  and ≥80% reuse (actual 91.6%). Honest caveats published IN the doc: LOC is a size
  proxy not person-hours (agent-built synthetic prototype), and credit's domain logic
  partly lives in platform (pre-DIP-pattern), so the credit-DIP figure understates
  domain #1 — which only strengthens the reuse story for domain #2.
